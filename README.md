...
3단계 하이퍼파라미터 최적화를 거쳐 달성한 최종 성능입니다.

| 메트릭 | Base 모델 | Fine-tuned (v3) |
|--------|-----------|-----------------|
| JSON 파싱률 | 0% | **100%** |
| 정확도 | 0% | **83.3%** |
| F1 (Macro) | 0% | **82.8%** |
...
| 버전 | Epochs | Target Modules | Eval Loss | 정확도 | 토픽 F1 |
|------|--------|---------------|-----------|--------|---------|
| v1 | 5 | Attention (4개) | 0.631 | 83.3% | 75~79% |
| v2 | 3 | +MLP (7개) | 0.496 | 80.0% | 80~84% |
| **v3** | **4** | **+MLP (7개)** | **0.480** | **83.3%** | **82~83%** |
...
│   └── generate_report_v3.py       # v3 최종 보고서 생성
├── docs/
...
`outputs/finetuning_report_v3.pdf`에 8페이지 분석 보고서가 생성됩니다.

---

## Training Details (v3 최종 설정)
...
- 3단계 최적화 과정 및 상세 분석은 `reports/generate_report_v3.py`로 생성되는 PDF 보고서를 참고하세요.
